...
async def pm_text(bot, message):
    ...
    await message.reply_text("<b>ඔයාගේ මැසේජ් එක ඇඩ්මින්ලාට ගියා !</b>")
    ...
    try:
        if settings['max_btn']:
            ...
            if n_offset == 0:
                ...
                btn.append([InlineKeyboardButton("⭕ පිටු", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages"), InlineKeyboardButton("𝐍𝐄𝐗𝐓 ➪", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
            ...
            if n_offset == 0:
                btn.append(
                    [InlineKeyboardButton("🌸 ආපසු", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total/int(MAX_B_TN))}", callback_data="pages")]
                )
            elif off_set is None:
                btn.append([InlineKeyboardButton("⭕ පිටු", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total/int(MAX_B_TN))}", callback_data="pages"), InlineKeyboardButton("𝐍𝐄𝐗𝐓 ➪", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
                        InlineKeyboardButton("🌸 ආපසු", callback_data=f"next_{req}_{key}_{off_set}"),
    ...
    if files:
        ...
        if offset != "":
            ...
            try:
                if settings['max_btn']:
                    btn.append(
                        [InlineKeyboardButton("⭕ පිටු", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="𝐍𝐄𝐗𝐓 ➪",callback_data=f"next_{req}_{key}_{offset}")]
                    )

                else:
                    btn.append(
                        [InlineKeyboardButton("⭕ පිටු", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/int(MAX_B_TN))}",callback_data="pages"), InlineKeyboardButton(text="𝐍𝐄𝐗𝐓 ➪",callback_data=f"next_{req}_{key}_{offset}")]
                    )
            except KeyError:
                await save_group_settings(query.message.chat.id, 'max_btn', True)
                btn.append(
        ...
        return await query.answer(f"සොරි අනේ, මට මේ නමින් කිසිම දෙයක් හොයන් බැනේ {movie}.", show_alert=True)
...
async def select_language(bot, query):
    ...
    if int(userid) not in [query.from_user.id, 0]:
        return await query.answer(script.ALRT_TXT.format(query.from_user.first_name), show_alert=True)
    btn = [[
        InlineKeyboardButton("ඔයාට ඔනි භාෂාව තොරගන්න↓", callback_data=f"lang#{userid}#unknown")
    ],[
        ...
        InlineKeyboardButton("ආපසු", callback_data=f"lang#{userid}#home")
    ]]
    try:
        await query.edit_message_reply_markup(
            reply_markup=InlineKeyboardMarkup(btn)
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if (st.status == enums.ChatMemberStatus.OWNER) or (str(userid) in ADMINS):
                ...
                await query.answer("මේක ඔයාගේ නෙමේ අනේ !!", show_alert=True)
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                if clicked == typed:
                    ...
                    return await query.answer(f"Hᴇʏ {query.from_user.first_name}, මේක ඔයාගේ එක නෙමේ ඔයාටත් ඔන්නම් මේකම search කරන්න !", show_alert=True)
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("අපේ Infinity Channel එකට join වෙන්න ! 🙂", show_alert=True)
            return
        ident, file_id = query.data.split("#")
        if file_id == "send_all":
        ...
        if query.from_user.id in ADMINS:
            ...
            try:
                await client.send_message(chat_id=int(from_user), text=f"<b>Hᴇʏ {user.mention}, ඔන්න ඔයාගේ ʀᴇᴏ̨ᴜᴇsᴛ එක සම්පුර්ණ කරලා තියෙන්නේ ආපහු search කරලා බලන්න.</b>", reply_markup=InlineKeyboardMarkup(btn2))
            except UserIsBlocked:
                await client.send_message(chat_id=int(SUPPORT_CHAT_ID), text=f"<b>Hᴇʏ {user.mention}, ඔන්න ඔයාගේ ʀᴇᴏ̨ᴜᴇsᴛ එක සම්පුර්ණ කරලා තියෙන්නේ ආපහු search කරලා බලන්න. Kɪɴᴅʟʏ sᴇᴀʀᴄʜ ᴀɢᴀɪɴ.\n\nNᴏᴛᴇ: Tʜɪs ᴍᴇssᴀɢᴇ ɪs sᴇɴᴛ ᴛᴏ ᴛʜɪs ɢʀᴏᴜᴘ ʙᴇᴄᴀᴜsᴇ ʏᴏᴜ'ᴠᴇ ʙʟᴏᴄᴋᴇᴅ ᴛʜᴇ ʙᴏᴛ. Tᴏ sᴇɴᴅ ᴛʜɪs ᴍᴇssᴀɢᴇ ᴛᴏ ʏᴏᴜʀ PM, Mᴜsᴛ ᴜɴʙʟᴏᴄᴋ ᴛʜᴇ ʙᴏᴛ.</b>", reply_markup=InlineKeyboardMarkup(btn2))
        else:
            await query.answer("Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢʜᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True)
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🌿 ꜱᴛᴏʀᴇꜱ 🌿', callback_data='store_file')
        ], [
            InlineKeyboardButton('🔰 ᴄᴏɴɴᴇᴄᴛ 🔰', callback_data='coct'),
            InlineKeyboardButton('🪴 ᴘʟᴜɢɪɴꜱ 🪴', callback_data='extra')
        ], [
        ...
        buttons = [[
            ...
            InlineKeyboardButton('its me', callback_data='source')
        ],[
